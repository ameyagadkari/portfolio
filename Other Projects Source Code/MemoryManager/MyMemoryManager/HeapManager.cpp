...
HeapManager * HeapManager::Create(void * heapMemory, size_t sizeHeap)
{
	if (heapManager == NULL)
	{
		...
		heapManager->startOfHeap = reinterpret_cast<uint8_t *>(heapMemory);
		heapManager->currentPosOfHeap = heapManager->startOfHeap;
		heapManager->endOfHeap = reinterpret_cast<uint8_t *>(heapMemory)+sizeHeap;
...
void *  HeapManager::myAlloc(size_t chunkSize)
{
	...
	if (memoryList == NULL && static_cast<size_t>(heapManager->endOfHeap - heapManager->currentPosOfHeap) >= ROUND_UP(sizeof(MemBlockDesc) + chunkSize, 4))
	{
		...
		memoryList->userStart = reinterpret_cast<uint8_t *>(memoryList)+sizeof(MemBlockDesc);
	...
	else
	{
		...
		do
		{
			...
			if (traverseTemp->fullSize >= ROUND_UP(sizeof(MemBlockDesc) + chunkSize, 4) && traverseTemp->isInUse == false)
			{
				...
				traverseTemp->userStart = reinterpret_cast<uint8_t *>(traverseTemp)+sizeof(MemBlockDesc);
				return traverseTemp->userStart;
			}
			tempTraverseTemp = traverseTemp->next;
		} while (traverseTemp->next != NULL);
		//first hand memory
		if (static_cast<size_t>(heapManager->endOfHeap - heapManager->currentPosOfHeap) >= ROUND_UP(sizeof(MemBlockDesc) + chunkSize, 4))
		{
...
{
	assert(startOfHeap<=pPtr && pPtr<=endOfHeap);
	MemBlockDesc * traverseTemp = head->next;
	MemBlockDesc * tempTraverseTemp = head->next;
	do
	{
		traverseTemp = tempTraverseTemp;
		if ((reinterpret_cast<MemBlockDesc *>(reinterpret_cast<uint8_t *>(pPtr)-sizeof(MemBlockDesc)) == traverseTemp) && traverseTemp->isInUse == true)
		{
...
void  HeapManager::myCollect()
{
	...
	{	
		...
		{
			traverseTemp = tempTraverseTemp;
			tempTraverseTemp2 = reinterpret_cast<MemBlockDesc *>(reinterpret_cast<uint8_t *>(traverseTemp)+traverseTemp->fullSize);
